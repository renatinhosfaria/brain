...
class MetaAdsModelsTests(unittest.TestCase):
    def test_constants_are_exact_read_only_boundary(self):
        self.assertEqual(
            META_READ_TOOLS,
            frozenset({"meta_list_ad_accounts", "meta_get_ad", "meta_get_campaign"}),
        )
        self.assertIn("meta_inactive", META_ERROR_CODES)

    def test_observer_source_preserves_decimal_string_exactly(self):
        result = observed_ctwa_source(
            {"sourceType": "ad", "sourceId": "00042", "ctwaClid": "clid"}
        )
    ...
    def test_observer_source_rejects_noncanonical_or_invalid_shapes(self):
        for raw in (
            ...
            [],
            {},
            {"sourceType": "campaign", "sourceId": "42"},
        ...
        self.assertEqual(
            observed_ctwa_source(
                {"sourceType": "ad", "sourceId": "42", "hash": "x"}
            ).source_id,
            "42",
        )
    ...
    def test_confirmation_requires_both_effective_statuses_active(self):
        with self.assertRaises(ValueError):
            ConfirmedMetaAttribution(
                "1", "Ad", "2", "Campaign", "ACTIVE", "PAUSED", "ACTIVE", "ACTIVE"
            )
        confirmed = ConfirmedMetaAttribution(
            "1", "Ad", "2", "Campaign", "PAUSED", "ACTIVE", "PAUSED", "ACTIVE"
        )
    ...
    def test_account_normalization_accepts_only_configured_forms(self):
        self.assertEqual(
            normalize_ad_account_id("1598606388477916"), "act_1598606388477916"
        )
        self.assertEqual(
            normalize_ad_account_id("act_1598606388477916"), "act_1598606388477916"
        )
        with self.assertRaises(ValueError):
