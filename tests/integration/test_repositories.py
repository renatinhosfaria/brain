...
async def test_list_memories_filtra_por_confianca_fonte_e_metadata(session):
    await repo.add_memory(
        session,
        namespace="p",
        content="memoria relevante",
        embedding=_vec(0.5),
        confidence=0.95,
        source="conversation",
        meta={"project": "brain", "kind": "fact"},
    )
    await repo.add_memory(
        session,
        namespace="p",
        content="memoria fraca",
        embedding=_vec(0.6),
        confidence=0.4,
        source="import",
        meta={"project": "outro"},
    ...
    mems = await repo.list_memories(
        session,
        "p",
        filters={
            "source": "conversation",
            "min_confidence": 0.8,
            "metadata": {"project": "brain"},
        },
    )

    assert [m.content for m in mems] == ["memoria relevante"]


async def test_merge_memories_preserva_superseded_e_oculta_por_padrao(session):
    target = await repo.add_memory(
        session, namespace="p", content="memoria consolidada", embedding=_vec(0.5)
    )
    duplicate = await repo.add_memory(
        session, namespace="p", content="memoria duplicada", embedding=_vec(0.51)
    )
    await session.commit()

    merged_into = await repo.merge_memories(session, [target.id, duplicate.id], target.id)
    await session.commit()

    assert merged_into == target.id
    superseded = await repo.get_memory(session, duplicate.id)
    assert superseded is not None
    assert superseded.supersedes_id == target.id
    active = await repo.list_memories(session, "p")
    assert [m.id for m in active] == [target.id]
    all_mems = await repo.list_memories(session, "p", filters={"include_superseded": True})
    assert {m.id for m in all_mems} == {target.id, duplicate.id}


async def test_search_memories_oculta_superseded_por_padrao(session):
    target = await repo.add_memory(
        session, namespace="p", content="memoria consolidada", embedding=_vec(0.5)
    )
    duplicate = await repo.add_memory(
        session, namespace="p", content="memoria duplicada", embedding=_vec(0.5)
    )
    await session.commit()
    await repo.merge_memories(session, [target.id, duplicate.id], target.id)
    await session.commit()

    results = await repo.search_memories(session, _vec(0.5), "p", limit=10)

    assert [r["ref"] for r in results] == [str(target.id)]


async def test_delete_memory_remove_superseded_dependentes(session):
    target = await repo.add_memory(
        session, namespace="p", content="memoria consolidada", embedding=_vec(0.5)
    )
    duplicate = await repo.add_memory(
    ...
    assert await repo.delete_memory(session, target.id) is True
    await session.commit()

    assert await repo.get_memory(session, target.id) is None
    assert await repo.get_memory(session, duplicate.id) is None
